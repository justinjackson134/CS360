...
main(int argc, char *argv[])
{
   ...
   while(1){
	...
	if(stat(pathName, &fileStat) < 0) {
	  ...
	  printf("STAT STRUCT FILE TYPE: %d\n", fileStat.st_mode);
	  printf("STAT STRUCT FILE SIZE: %d\n", fileStat.st_size);
	  if(S_ISREG(fileStat.st_mode)) {
	    printf("STAT STRUCT IS REG\n");

	    // KCS COUNT  
	    //open filename for READ
	    int fp = open(pathName, O_RDONLY);

	    sprintf(toSend, "%d", fileStat.st_size);
	    
	    printf("SENDING SIZE...\n");
	    n = write(client_sock, toSend, MAX);
	    ...
	    printf("STAT STRUCT NOT REG\n");

	    sprintf(toSend, "BAD");
	    
	    printf("SENDING BAD INDICATOR...\n");
	    n = write(client_sock, toSend, MAX);
	    printf("server: wrote n=%d bytes; ECHO=[%s]\n", n, toSend);
	  }
	}
