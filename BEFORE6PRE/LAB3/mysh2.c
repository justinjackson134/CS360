...
void parseCommand(char* command, char** commandArgs)
{
  ...
  do {                                          // Do until no more args
    if(i == 0) {                                // If on first element, it is the command
      ...
      //DEBUG PRINT// printf("  Arg: %s\n", commandArgs[i]);    // Echo args per pipe
      if(strcmp(commandArgs[i-1], "<") == 0) {  // If we have an infile last arg, we now have infile name
	FILE *infile;                           // Used to store infile

	infile = fopen(commandArgs[i], "r");    // Open infile given name
	if(infile == NULL) {                    // If we failed to open,
	  printf("\nFAILED TO OPEN FILE\n");     // Print error message
	}
	else {                                  // If we succeeded in opening our file,
	  commandArgs[i] = NULL;                // Remove filename
	  i--;                                  // Deincrement counter and remove the < and filename
	  commandArgs[i] = NULL;                // Remove <
	  char fileLine[1024];                  // used to store line from file
	  fgets(fileLine, 1024, infile);        // read line from file
	  ...
	  char buffer[1024];
	  do {
	    if(fileLine[j] != ' ' && fileLine[j] != '\0') { // If we are not reading whitespace, appending cmd
	      buffer[k] = fileLine[j];
	      printf("BUFFER: %s\n", buffer);
	      k++;
	    }
	    else {                              // We are reading whitespace
	      printf("APPEND: '%s' to '%s'\n", buffer, commandArgs[i]);
	      commandArgs[i] = (char*)malloc(sizeof(char)*1024);
	      sprintf(commandArgs[i], "%s", buffer);
	      memset(buffer, '\0', 1024);
	      i++;
	      k = 0;
	    }
	    j++;
	  } while(j <= strlen(fileLine));
	}
      }      
    }    
    i++;                                        // Increment counter
...
{           
  ...
  char outputBuffer[2048];                      // Used to hold output temporarily
